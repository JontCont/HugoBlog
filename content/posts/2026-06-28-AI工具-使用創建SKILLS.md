...
> 💡 **現實痛點**：每個工具的格式與路徑各行其道，若手動在多個工具間複製貼上，不僅耗時更極易造成版本不一致。這就是後續我們需要 `rules-bridge` 的原因。

---

## 五、實戰篇（一）：從零建立客製化 Skill
...
## 六、實戰篇（二）：安裝與管理現成社群 Skills

除了自己寫，你也可以直接安裝開源社群維護的高品質技能包。
...
安裝過程中，終端機會以互動式介面詢問你要安裝到哪些 AI 工具（如 GitHub Copilot, Antigravity, Cursor 等），並自動建立 Symlink 或複製檔案。

### 2. 常用技能市集（Marketplace）
- **[GitHub awesome-copilot](https://awesome-copilot.github.com/)**：收錄社群針對 Copilot CLI 開發的實用技能。
- **[Claude Marketplaces](https://claudemarketplaces.com/marketplaces)**：提供多種適用於 Anthropic Claude Agent 的擴充模組。

### 3. Copilot CLI 實用管理指令
在 GitHub Copilot CLI 中，你可以透過以下指令隨時管理本機技能：

```bash
# 列出所有已安裝並啟用的 Skills
/skills list

# 查看特定 Skill 的詳細描述與中繼資料
/skills info <SKILL_NAME>
```

---

## 七、進階篇：透過 rules-bridge 實現跨工具一鍵同步

為了消除各大 AI 工具之間的路徑與格式隔閡，開源專案 **[rules-bridge](https://github.com/JontCont/rules-bridge)** 提供了「單一來源（Single Source of Truth）」的跨工具同步方案。

```mermaid
flowchart LR
    A[根目錄 AGENTS.md / .agents/skills/] -->|/rules-bridge all| B(rules-bridge 轉換引擎)
    B --> C[GitHub Copilot: .github/]
    B --> D[Cursor: .cursor/rules/]
    B --> E[Codex / Antigravity: .agents/]
```

### 1. 核心工作原理
只需在專案根目錄維護一份標準的 `AGENTS.md` 與 `.agents/skills/`，執行指令後，腳本會自動完成跨平台適配：
- **GitHub Copilot**：自動複製至 `.github/copilot-instructions.md` 並鏡像同步技能目錄。
- **Cursor**：自動封裝成帶有 `alwaysApply: true` YAML 前綴的 `.cursor/rules/base.mdc`。
- **Antigravity / Codex**：確保原生目錄結構齊全並即時生效。
...
幾秒內即可看到所有平台的同步成功勾選標記 `✅`，徹底告別手動維護多份規則的惡夢！

---

## 八、開發 Skills 的最佳實踐（Best Practices）與避坑指南
